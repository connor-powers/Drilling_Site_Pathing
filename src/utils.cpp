...
double weighted_cost_function(double input_site_reward_function_val,
                              double input_distance_to_site,
                              double input_distance_weight) {
  return (-input_site_reward_function_val +
          input_distance_to_site * input_distance_weight);
}
...
double distance(std::pair<double, double> input_coords_1,
  ...
  return std::sqrt(std::pow(input_coords_2.first - input_coords_1.first, 2) +
                   std::pow(input_coords_2.second - input_coords_1.second, 2));
}
...
double calc_cost_function_from_position_to_site(
  ...
  double calculated_distance =
      distance(current_position, target_site.coordinates);
  double total_cost = weighted_cost_function(
      target_site.reward_val, calculated_distance, input_distance_weight);
...
double calculate_total_distance_from_sequence(
  ...
  // first site
  double calculated_distance =
      distance(input_starting_position, input_site_sequence.at(0).coordinates);
  double total_distance = calculated_distance;

  std::pair<double, double> current_position =
      input_site_sequence.at(0).coordinates;

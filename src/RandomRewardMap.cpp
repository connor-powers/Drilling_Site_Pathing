...
void RandomRewardMap::identify_potential_sites(
    int input_desired_num_sites, double input_site_reward_val_threshold) {
  // Populates the list_of_sites_ with the <input_desired_num_sites> sites with
  // the highest reward vals above the input threshold
  ...
  std::cout << "Max observed reward val after scaling: "
            << max_observed_reward_val << "\n";
  std::cout << "Min observed reward val after scaling: "
            << min_observed_reward_val << "\n";
  ...
  std::vector<site> trimmed_site_list(
      output_list_of_sites.begin(),
      output_list_of_sites.begin() + input_desired_num_sites);
...
void RandomRewardMap::identify_potential_sites(
    double input_site_reward_val_threshold) {
  // If no input_num_sites is passed to the function, then populates
  // list_of_sites_ with all sites with reward vals greater than or equal to
  // threshold val
  std::vector<site> output_list_of_sites = {};
...
void RandomRewardMap::draw_map() {
  ...
  if (gnuplot_pipe) {
    ...
    fprintf(gnuplot_pipe,
            "set palette defined (%f 'white', %f 'forest-green')\n",
...
RandomRewardMap::generate_paths_distance_weighted_NN(
  ...
  while (unvisited_sites.size() > 0) {
    ...
    // from the current position, taking into account distance and reward
    // function
...
void RandomRewardMap::draw_map_with_paths(
  ...
  if (gnuplot_pipe) {
    ...
    fprintf(gnuplot_pipe, "set yrange [%f:%f]\n", min_y_, max_y_);
    fprintf(gnuplot_pipe,
            "set cblabel 'Reward Function' font 'Times,14' offset 1\n");
    fprintf(gnuplot_pipe, "set tics font 'Times,14'\n");
...
void RandomRewardMap::plot_NN_total_distance_swept_distance_weight(

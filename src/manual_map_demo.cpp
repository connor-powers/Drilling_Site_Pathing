...
int main() {
  ...
  // You can also add sites by just entering their coordinates and reward value.
  // Format: (x,y,reward_val)
  ...
  std::uniform_real_distribution<> site_x_distribution(sites_x_lower_bound,
                                                       sites_x_upper_bound);
  std::uniform_real_distribution<> site_y_distribution(sites_y_lower_bound,
                                                       sites_y_upper_bound);
  std::uniform_real_distribution<> site_reward_val_distribution(
      sites_reward_val_lower_bound, sites_reward_val_upper_bound);
  ...
  manual_reward_map.add_site(site_x_distribution(engine),
                             site_y_distribution(engine),
                             site_reward_val_distribution(engine));
  manual_reward_map.add_site(site_x_distribution(engine),
                             site_y_distribution(engine),
                             site_reward_val_distribution(engine));
  ...
  std::pair<std::vector<site>, double> paths_and_distance_descending_priority =
      manual_reward_map.generate_paths_descending_priority();
  std::vector<site> paths_descending_priority =
      paths_and_distance_descending_priority.first;
  double distance_descending_priority =
      paths_and_distance_descending_priority.second;
  manual_reward_map.draw_map_with_paths(paths_descending_priority,
                                        "DescendingPriority");
  std::cout << "Total distance of descending priority method: "
            << distance_descending_priority << "\n";

  double distance_weight_med = 1;
  std::pair<std::vector<site>, double> paths_and_distance_NN_midweight =
      manual_reward_map.generate_paths_distance_weighted_NN(
          distance_weight_med);
  std::vector<site> paths_NN_midweight = paths_and_distance_NN_midweight.first;
  double distance_NN_midweight = paths_and_distance_NN_midweight.second;
  manual_reward_map.draw_map_with_paths(paths_NN_midweight, "Weighted_NN",
                                        distance_weight_med);
  std::cout << "Total distance of NN method with distance weight = "
            << distance_weight_med << ": " << distance_NN_midweight << "\n";

  double distance_weight_high = 100;
  std::pair<std::vector<site>, double> paths_and_distance_NN_highweight =
      manual_reward_map.generate_paths_distance_weighted_NN(
          distance_weight_high);
  std::vector<site> paths_NN_highweight =
      paths_and_distance_NN_highweight.first;
  double distance_NN_highweight = paths_and_distance_NN_highweight.second;
  manual_reward_map.draw_map_with_paths(paths_NN_highweight, "Weighted_NN",
                                        distance_weight_high);
  std::cout << "Total distance of NN method with distance weight = "
            << distance_weight_high << ": " << distance_NN_highweight << "\n";

  // Let's try sweeping this distance_weight and see if there's a noticeable
  // minimum of the total distance
  double weight_increment = 0.001;
  size_t num_weight_increments = 100000;
  manual_reward_map.plot_NN_total_distance_swept_distance_weight(
      weight_increment, num_weight_increments);
